...
const alertFunc = (event) => {
  if (event.target.classList.contains("important")) {
    alert("The most important franchise ever, the story of DOM(inic) Toretto's family. It's not about car, it'çs about family.")
  }
  else {
...
let option1 = new Option("important franchises");
let option2 = new Option("normal franchises");
...
const display = () => {
   if (select1.selectedIndex === 1) {
        //fF.style.visibility = "visible"; + a=1
        for (a=0; a < childrenLi.length; a++) {
         if (!childrenLi[a].className) {
          childrenLi[a].style.visibility = "hidden";
          }
        }
    }
    else if (select1.selectedIndex === 0) {
      for (a=0; a < childrenLi.length; a++) {
        childrenLi[a].style.visibility = "visible";
      }
    }
  }
